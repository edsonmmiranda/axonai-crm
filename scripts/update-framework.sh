#!/usr/bin/env bash
# Sincroniza o projeto atual com a versão mais recente do axonai-framework.
#
# Uso (dentro de um projeto derivado do template):
#   bash scripts/update-framework.sh              # pega o branch main
#   bash scripts/update-framework.sh v1.2.0       # pega uma tag/branch específica
#
# O script:
#   1. Clona o framework em um diretório temporário
#   2. Sobrescreve SOMENTE as pastas "do framework" no projeto atual
#   3. NUNCA toca em código, sprints, migrations de feature, .env, etc.
#   4. Mostra git status no final — você revisa e decide o que commitar
...
echo "🔄 Sincronizando pastas do framework..."
...
echo ""
echo "✨ Sync concluído. Revise as mudanças:"
echo ""
cd "$PROJECT_ROOT"
git status --short
...
echo "  git commit -m 'chore: sync framework updates'"
