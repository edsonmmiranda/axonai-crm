...
import { getWhatsappGroupByIdAction } from '@/lib/actions/whatsapp-groups';
...
export default async function EditWhatsappGroupPage(props: {
  ...
  const res = await getWhatsappGroupByIdAction(id);
    ...
    <div className="mr-auto flex max-w-page flex-col gap-6 pb-10">
      <nav className="flex text-sm font-medium text-text-secondary" aria-label="breadcrumb">
        <ol className="flex items-center gap-2">
          ...
          <li className="truncate font-semibold text-text-primary" title={group.name}>
            {group.name}
          </li>
        </ol>
      </nav>
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-4 text-sm text-text-secondary">
          ...
          <span className="flex items-center gap-1.5">
            ...
            Atualizado em {formatDate(group.updated_at)}
          </span>
        </div>
      </div>

      <WhatsappGroupForm mode="edit" group={group} isAdmin />
    </div>
  );
}
