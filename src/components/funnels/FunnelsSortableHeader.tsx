...
import type { FunnelSortKey } from '@/lib/funnels/constants';
import { parseSortParam, serializeSortParam } from './sort-utils';

interface FunnelsSortableHeaderProps {
...
export function FunnelsSortableHeader({
  sortKey,
  label,
  align = 'left',
  className,
}: FunnelsSortableHeaderProps) {
  ...
  function onToggle() {
    ...
    startTransition(() => {
      router.push(qs ? `/funnels?${qs}` : '/funnels');
    });
  }
      ...
      >
        ...
        <span
          className={cn(
            ...
            rule
              ? 'text-action-primary opacity-100'
