...
import type { FunnelRow } from '@/lib/actions/funnels';
import { FunnelRowActions } from './FunnelRowActions';
import { FunnelsSortableHeader } from './FunnelsSortableHeader';
...
export function FunnelsList({ funnels, hasFilter, metadata }: FunnelsListProps) {
  ...
  if (funnels.length === 0) {
    if (hasFilter) {
      return (
        <div className="flex flex-col items-center gap-2 px-6 py-12 text-center">
          <p className="text-sm font-medium text-text-primary">Nenhum funil encontrado</p>
    ...
    return (
      <div className="flex flex-col items-center gap-3 px-6 py-12 text-center">
        <p className="text-sm font-medium text-text-primary">Nenhum funil cadastrado</p>
        <p className="text-sm text-text-secondary">
          Crie seu primeiro funil para configurar o pipeline de vendas.
        </p>
        <Button asChild className="mt-2">
          <Link href="/funnels/new">Criar primeiro funil</Link>
        </Button>
      </div>
    );
  }

  const total = metadata?.total ?? funnels.length;
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-left text-sm">
          <thead className="border-b border-border-subtle bg-surface-sunken text-xs uppercase text-text-secondary">
            <tr>
              <FunnelsSortableHeader sortKey="name" label="Nome" className="pl-6 pr-3" />
              <th
                scope="col"
              ...
              <FunnelsSortableHeader sortKey="is_active" label="Status" />
              <FunnelsSortableHeader sortKey="created_at" label="Criado em" />
              <th
                scope="col"
            ...
            {funnels.map((funnel) => (
              ...
              >
                ...
                <td className="whitespace-nowrap px-3 py-4 text-text-secondary">
                  {funnel.stage_count}{' '}
                  {funnel.stage_count === 1 ? 'estágio' : 'estágios'}
                </td>
