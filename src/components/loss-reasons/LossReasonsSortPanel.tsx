...
export function LossReasonsSortPanel() {
  ...
  const [pendingDir, setPendingDir] = useState<LossReasonSortDir>('asc');

  const availableKeys = LOSS_REASON_SORT_KEYS.filter(
    (k) => !rules.some((r) => r.key === k),
  );

  function commit(next: LossReasonSortRule[]) {
  ...
  function togglePillDir(key: LossReasonSortKey) {
    const next = rules.map((r) =>
      r.key === key ? { ...r, dir: r.dir === 'asc' ? 'desc' : 'asc' } : r,
    ) as LossReasonSortRule[];
    commit(next);
  }

  function removeRule(key: LossReasonSortKey) {
  ...
  function addRule() {
    ...
    if (!LOSS_REASON_SORT_KEYS.includes(pendingKey as LossReasonSortKey)) return;
    const key = pendingKey as LossReasonSortKey;
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-surface-raised shadow-sm">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        aria-controls="loss-reasons-sort-body"
          ...
          {availableKeys.length > 0 ? (
            <div className="flex flex-wrap items-center gap-2 border-t border-border-subtle pt-3">
              ...
              <div className="w-full sm:w-48">
                <Select
                  value={pendingDir}
                  onValueChange={(v) => setPendingDir(v as LossReasonSortDir)}
                >
                  <SelectTrigger className="h-9">
                    <SelectValue />
                  </SelectTrigger>
