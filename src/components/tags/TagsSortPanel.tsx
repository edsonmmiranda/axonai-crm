...
export function TagsSortPanel() {
  ...
  const [pendingDir, setPendingDir] = useState<TagSortDir>('asc');

  const availableKeys = TAG_SORT_KEYS.filter(
    (k) => !rules.some((r) => r.key === k),
  );

  function commit(next: TagSortRule[]) {
  ...
  function togglePillDir(key: TagSortKey) {
  ...
  function removeRule(key: TagSortKey) {
  ...
  function addRule() {
    ...
    if (!TAG_SORT_KEYS.includes(pendingKey as TagSortKey)) return;
    const key = pendingKey as TagSortKey;
          ...
          {availableKeys.length > 0 ? (
            <div className="flex flex-wrap items-center gap-2 border-t border-border-subtle pt-3">
              ...
              <div className="w-full sm:w-48">
                <Select
                  value={pendingDir}
                  onValueChange={(v) => setPendingDir(v as TagSortDir)}
                >
                  <SelectTrigger className="h-9">
                    <SelectValue />
                  </SelectTrigger>
