...
import { TagsSortableHeader } from './TagsSortableHeader';
...
export function TagsList({ tags, hasFilter, metadata }: TagsListProps) {
  ...
  const total = metadata?.total ?? tags.length;
        ...
        <table className="min-w-full text-left text-sm">
          <thead className="border-b border-border-subtle bg-surface-sunken text-xs uppercase text-text-secondary">
            <tr>
              ...
              <TagsSortableHeader sortKey="name" label="Nome" />
              <TagsSortableHeader sortKey="is_active" label="Status" />
              <TagsSortableHeader sortKey="created_at" label="Criado em" />
              <th
                scope="col"
          ...
          <tbody className="divide-y divide-border-subtle">
            {tags.map((tag) => (
              ...
              >
                ...
                <td className="whitespace-nowrap px-3 py-4">
                  <Badge variant={tag.is_active ? 'role-admin' : 'status-inactive'}>
