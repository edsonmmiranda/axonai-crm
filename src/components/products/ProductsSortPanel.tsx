...
export function ProductsSortPanel() {
  ...
  const [pendingDir, setPendingDir] = useState<ProductSortDir>('asc');

  const availableKeys = PRODUCT_SORT_KEYS.filter(
    (k) => !rules.some((r) => r.key === k)
  );

  function commit(next: ProductSortRule[]) {
  ...
  function togglePillDir(key: ProductSortKey) {
  ...
  function removeRule(key: ProductSortKey) {
  ...
  function addRule() {
    ...
    if (!PRODUCT_SORT_KEYS.includes(pendingKey as ProductSortKey)) return;
    const key = pendingKey as ProductSortKey;
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-surface-raised shadow-sm">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        aria-controls="products-sort-body"
          ...
          {availableKeys.length > 0 ? (
            <div className="flex flex-wrap items-center gap-2 border-t border-subtle pt-3">
              ...
              <div className="w-full sm:w-48">
                <Select
                  value={pendingDir}
                  onValueChange={(v) => setPendingDir(v as ProductSortDir)}
                >
                  <SelectTrigger className="h-9">
                    <SelectValue />
                  </SelectTrigger>
