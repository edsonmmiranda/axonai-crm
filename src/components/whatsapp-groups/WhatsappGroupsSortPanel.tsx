...
export function WhatsappGroupsSortPanel() {
  ...
  const [pendingDir, setPendingDir] = useState<WhatsappGroupSortDir>('asc');

  const availableKeys = WHATSAPP_GROUP_SORT_KEYS.filter(
    (k) => !rules.some((r) => r.key === k),
  );

  function commit(next: WhatsappGroupSortRule[]) {
  ...
  function togglePillDir(key: WhatsappGroupSortKey) {
    const next = rules.map((r) =>
      r.key === key ? { ...r, dir: r.dir === 'asc' ? 'desc' : 'asc' } : r,
    ) as WhatsappGroupSortRule[];
    commit(next);
  }

  function removeRule(key: WhatsappGroupSortKey) {
  ...
  function addRule() {
    ...
    if (!WHATSAPP_GROUP_SORT_KEYS.includes(pendingKey as WhatsappGroupSortKey)) return;
    const key = pendingKey as WhatsappGroupSortKey;
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-surface-raised shadow-sm">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        aria-controls="groups-sort-body"
          ...
          {availableKeys.length > 0 ? (
            <div className="flex flex-wrap items-center gap-2 border-t border-border-subtle pt-3">
              ...
              <div className="w-full sm:w-48">
                <Select
                  value={pendingDir}
                  onValueChange={(v) => setPendingDir(v as WhatsappGroupSortDir)}
                >
                  <SelectTrigger className="h-9">
                    <SelectValue />
                  </SelectTrigger>
