...
import type { WhatsappGroupRow } from '@/lib/actions/whatsapp-groups';
import { WhatsappGroupRowActions } from './WhatsappGroupRowActions';
import { WhatsappGroupsSortableHeader } from './WhatsappGroupsSortableHeader';
...
function buildPageUrl(searchParams: URLSearchParams, page: number): string {
  ...
  return qs ? `/whatsapp-groups?${qs}` : '/whatsapp-groups';
}
...
export function WhatsappGroupsList({ groups, hasFilter, metadata }: WhatsappGroupsListProps) {
  ...
  if (groups.length === 0) {
    ...
    return (
      <div className="flex flex-col items-center gap-3 px-6 py-12 text-center">
        ...
        <p className="text-sm text-text-secondary">
          Cadastre o primeiro grupo de WhatsApp da sua organização.
        </p>
        <Button asChild className="mt-2">
          <Link href="/whatsapp-groups/new">Cadastrar primeiro grupo</Link>
        </Button>
      </div>
    );
  }

  const total = metadata?.total ?? groups.length;
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-left text-sm">
          <thead className="border-b border-border-subtle bg-surface-sunken text-xs uppercase text-text-secondary">
            <tr>
              <WhatsappGroupsSortableHeader sortKey="name" label="Nome" className="pl-6 pr-3" />
              <WhatsappGroupsSortableHeader sortKey="description" label="Descrição" />
              <WhatsappGroupsSortableHeader sortKey="whatsapp_id" label="WhatsApp ID" />
              <WhatsappGroupsSortableHeader sortKey="is_active" label="Status" />
              <WhatsappGroupsSortableHeader sortKey="created_at" label="Criado em" />
              <th
                scope="col"
            ...
            {groups.map((group) => (
              ...
              >
                ...
                <td className="whitespace-nowrap px-3 py-4">
                  <Badge variant={group.is_active ? 'role-owner' : 'status-inactive'}>
        ...
        {totalPages > 1 ? (
          ...
          >
            ...
            {pageItems.map((item) => {
              ...
              if (isActive) {
                return (
                  <span key={item} className={cn(btnBase, btnActive)} aria-current="page">
                    {item}
                  </span>
                );
              }
